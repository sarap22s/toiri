      ...
      <div className="-mx-1 flex gap-1.5 overflow-x-auto px-1 pb-2">
        {PROMPT_CATEGORIES.map((c) => (
          <button
            ...
            className={`press relative flex shrink-0 items-center gap-1.5 rounded-md border px-3 py-1.5 text-[12px] ${
              c.id === active
      ...
      <motion.div key={active} className="mt-2 grid gap-2">
        {category.ideas.map((idea, i) => (
          <motion.button
            ...
            className="lift group flex items-start justify-between gap-3 rounded-lg border border-border bg-ink-800/40 px-3.5 py-3 text-left hover:border-primary/35 hover:bg-ink-800/80 hover:shadow-[0_14px_30px_-22px] hover:shadow-primary/70"
          >
